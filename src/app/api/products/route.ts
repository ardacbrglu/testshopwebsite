...
type DbRow = { slug: string; name: string; description: string; imageUrl: string | null; price: number; currency: string | null };
...
export async function GET(req: Request) {
  ...
  const shape = (p: DbRow) => {
    ...
    const active = isDiscountActiveFor(p.slug, { scope, tokenActive: ref.active, landingSlug: ref.landingSlug, preview: ref.preview });
  ...
  if (slug) {
    const p = await prisma.product.findFirst({
      ...
      select: { slug: true, name: true, description: true, imageUrl: true, price: true, currency: true },
    }) as DbRow | null;
