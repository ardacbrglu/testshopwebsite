...
type CheckoutBody = { items: ClientCartItem[]; email: string };

function isCheckoutBody(x: unknown): x is CheckoutBody {
  if (!x || typeof x !== "object") return false;
  const obj = x as Record<string, unknown>;
  if (!Array.isArray(obj.items)) return false;
  if (typeof obj.email !== "string") return false;

  // items elemanlarını da doğrula
  for (const it of obj.items) {
    if (!it || typeof it !== "object") return false;
    const r = it as Record<string, unknown>;
    if (typeof r.slug !== "string") return false;
    if ("quantity" in r && typeof r.quantity !== "number") return false;
  }
  return true;
}
...
export async function POST(req: Request) {
  try {
    ...
    for (const it of items) {
      ...
      const active = isDiscountActiveFor(row.slug, {
        scope,
    ...
    if (ref.active && caboItems.length > 0) {
      if (!webhook || !keyId || !secret) {
        ...
        const sig = hmacSha256Hex(secret, `${ts}.${rawBody}`); // [CABO-INTEGRATION]
        ...
        try {
          const res = await fetch(webhook, {
            ...
            headers: {
              ...
              "X-Cabo-Key-Id": keyId, // [CABO-INTEGRATION]
              "X-Cabo-Timestamp": String(ts),
              "X-Cabo-Signature": sig,
              "X-Key-Id": keyId, // back-compat
