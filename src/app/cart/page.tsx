...
export default function CartPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/products", { cache: "no-store" })
      .then(r => r.json()).then(d => setCatalog(d.data || []));
  }, []);

  const rows = useMemo(() => cart.map(ci => {
    const p = catalog.find((x) => x.slug === ci.slug);
    if (!p) return null;
    const line = Math.round(p.unitFinal * ci.quantity * 100) / 100;
    return { ...ci, product: p, line };
  }).filter(Boolean) as Array<{ slug: string; quantity: number; product: Product; line: number }>, [cart, catalog]);
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      {rows.length === 0 ? (
        <div className="text-white/70">Sepet boş. <Link href="/products" className="underline">Ürünlere dön</Link></div>
      ) : (
        <>
          <div className="space-y-4">
            {rows.map(r => (
              <div key={r.slug} className="flex items-center gap-4 rounded-xl border border-white/10 p-3">
                ...
                <button onClick={() => removeOne(r.slug)} className="rounded-lg border border-white/20 px-3 py-1 hover:bg-white/10">Sil</button>
              </div>
            ))}
          </div>

          <div className="mt-6 flex items-center justify-between">
            <div className="text-lg">Ara Toplam: <span className="font-semibold">{money(total, currency)}</span></div>
