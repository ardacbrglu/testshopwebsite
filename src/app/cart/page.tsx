...
export default async function CartPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (cartId) {
    ...
    items = await query(
      `SELECT ci.id AS cart_item_id, ci.quantity, p.id AS product_id, p.slug, p.name, p.price, p.imageUrl
  ...
  // ---- Toast mesajını sayfa yüklenince gönder
  const toastScript = (() => {
    const map:any = {
    ...
    const key = ["added","updated","removed","cleared","email","invalid_email"].find(k => sp?.[k]);
    if (!key) return "";
    const text = map[key];
    return `
      window.addEventListener("load",()=> {
        window.dispatchEvent(new CustomEvent("toast",{ detail:{ type:"success", text:${JSON.stringify(text)} } }));
    ...
    <main className="mx-auto max-w-5xl px-4 py-8">
      ...
      {haveItems && (
        <div className="space-y-4">
          {items.map(it=>(
            <div key={it.cart_item_id} className="flex items-center gap-4 p-4 rounded-xl bg-neutral-900 border border-neutral-800">
              <img src={it.imageUrl} alt={it.name} className="w-24 h-24 object-cover rounded-lg" />
              ...
              <form action="/api/cart" method="post" className="flex items-center gap-2">
                ...
                <input name="qty" type="number" min={1} defaultValue={String(it.quantity)}
                       className="bg-neutral-900 border border-neutral-800 rounded-xl px-2 py-1 w-20" />
                <button type="submit" className="px-3 py-2 rounded-xl bg-white text-black text-sm cursor-pointer hover:opacity-90">
                  Güncelle
                </button>
              </form>
              ...
              <form action="/api/cart" method="post">
                ...
                <input type="hidden" name="cart_item_id" value={String(it.cart_item_id)} />
                <button type="submit" className="px-3 py-2 rounded-xl bg-red-600 text-white text-sm cursor-pointer hover:opacity-90">
                  Sil
                </button>
              </form>
            </div>
          ...
          <div className="flex justify-between items-center mt-4">
            <form action="/api/cart" method="post">
              ...
              <button type="submit" className="px-3 py-2 rounded-xl bg-neutral-800 text-white text-sm cursor-pointer hover:bg-neutral-700">
                Sepeti temizle
              </button>
            </form>
            <div className="text-lg">Ara toplam: <b>{fmtTRY(subtotal)}</b></div>
          </div>
          ...
          <div className="mt-6 p-4 rounded-xl bg-neutral-900 border border-neutral-800">
            ...
            <form action="/api/cart" method="post" className="flex items-center gap-2">
              ...
              <input name="email" defaultValue={email ?? ""} placeholder="ornek@eposta.com"
          ...
          <form action="/api/checkout" method="post" className="mt-6">
            ...
            <button type="submit" className="px-4 py-3 rounded-xl bg-green-500 text-black font-medium cursor-pointer hover:opacity-90 disabled:opacity-40"
                    disabled={!email || !haveItems}>
              Satın al
            </button>
            {!email && <div className="text-sm text-red-400 mt-2">Satın almak için önce e-posta giriniz.</div>}
          </form>
        </div>
      )}
