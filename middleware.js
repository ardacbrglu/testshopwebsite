...
export async function middleware(req) {
  ...
  const secret = process.env.TESTSHOP_COOKIE_TTL_DAYS || process.env.CABO_HMAC_SECRET || "dev-secret"; // typo fix below
  const trueSecret = process.env.TESTSHOP_COOKIE_SECRET || process.env.CABO_HMAC_SECRET || "dev-secret";

  const payload = JSON.stringify({ ver: 1, ref, lid, scope, landingProduct, ts: Date.now() });
  const sig = await hmacSHA256(trueSecret, payload);
  ...
  res.cookies.set("cabo_attrib", value, { httpOnly: true, secure: true, sameSite: "Lax", path: "/", maxAge: days * 86400 });
  return res;
}
